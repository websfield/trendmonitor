...
- **Additive-only where possible.** Stage 0 owns the one deliberate breaking bump; this stage's additions to `content-package-v1` must be optional/minor, or the stability clock resets a second time. **[R1-fix]** *(Amended 2026-08-10, D-62: Stage 0's bump landed as `render-v2` only — `content-package-v2` was never created, its identity motive having dissolved into D-56 — so this stage's additive fields land on `content-package-v1`.)*
...
| 11 | Thread `objective`, `primaryMetric` and `changedVariable` through the editorial chain into the package (**additive/minor**, not a breaking bump; targets `content-package-v1` — D-62, no v2 exists) | general-purpose | `cutdown/packages/editorial/**`, `creative-brief-v*.json`, `content-package-v1.json` |
