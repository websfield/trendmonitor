...
| ~~`render-v1` path patterns (Phase 5 residual 1, BREAKING)~~ | ~~**Stage 0**~~ — **discharged 2026-08-11 by Stage 0B-3** (`render-v2.json`, D-62) |
| `render-manifest-v1.captions.*Path` + `source-asset-v1.storedPath` pattern-tightening (D-62b — same class, deliberately not bundled into 0B-3); spike residual 5 (`Role3` unpinned against codegen renumbering — the schemas Stage 5 adds are the renumbering kind); 0B residual 10 (a semantic change under `schemas/common/` or `enums/` is invisible to criterion 3 — `currentContractSet()` records only the 15 top-level contracts) | **Stage 5** — its bump re-plan inherits all three by D-62's revisit trigger |
...
| **0B-3** | **Complete (Ready — Almost, 4 residuals)** | `docs/progress/cutdown-product-program-stage-0b3-review.md`. **The bump landed as `render-v2` only (D-62)** — `content-package-v2` was never created (identity motive dissolved into D-56; Stage 1 additive by its own plan), and the deferred pattern-tightening (render-manifest/source-asset) moved to the §6 ledger with Stage 5 receiving. Constraint check at landing: window held **1**/10 resolved outputs (≤3 threshold). Reader-dispatch-before-writer (`readVersionedContractJson`, declared-major, no cross-major retry — premise asserted by test); writer stamps from `RENDER_SCHEMA_VERSION` (D-52 mechanism, drift-tested). Entry gate: **986 TS = 981 pass + 5 skipped + 0 fail** (baseline 974 = 969+5+0; +12 = exactly this stage's tests), **689 Python unchanged (delta 0, as pinned)**, `validate:contracts` **50 cases / 0 / 0 disagreements** (47 at entry gate; the code gate added three invalid fixtures for the captions siblings, backslash and trailing-newline classes), `skills sync --check` 10, `doctor` 7/7, ruff clean. **Live `status --phase0` byte-identical before/after** — criterion 3 flips `not_met` only on the next minted package, by design, healing earliest at the 11th resolved output overall. Plan gate: boundary **BLOCK** (writer misidentified — the envelope literal lives in `renderer-ffmpeg`'s adapter, which cannot import `skill-runtime`; fixed via the D-52 constant) then both reviewers all-RESOLVED round 2; `plan-reviewer` READY. **Stage 0 engineering is now complete**; A7 (CI green on a clean clone) remains open — no `gh` in any session yet — and T-2/T-3/T-4 remain owner-blocked. **Accumulation is safe to resume** (the §7 Stage-0 **Order-critical** clause, discharged by landing) |
...
| 2026-08-10 | **Stage 0B-3 plan gate**: `cutdown-boundary-reviewer` + `cutdown-measurement-reviewer` (rounds 1–2), then `plan-reviewer` | **BLOCK** + **NEEDS CHANGES**, then all r1 findings **RESOLVED** (2 + 3 residual CHANGEs applied as rev 3), then **READY** | The gate earned its keep in the plan's own *evidence base*: the boundary BLOCK proved the writer-move task named a file that does not write the render envelope — the real stamper is a hand-built literal in `packages/renderer-ffmpeg/src/adapter.ts:485-491`, in a package that **cannot import** `skillEnvelope` (the 2026-07-30 package-boundary lesson, verbatim) — and the fix adopted the D-52 shared-constant + drift-test mechanism. The measurement rounds killed five unreconciled numbers, including a phantom "18 lint rules" born in the 0B spike (the artefact defines 12 rule ids; gate now stated count-free, spike corrected in place) and a pytest pin over a population pytest does not read; round 2 additionally pinned B4's sole-stamper grep key after showing the plan's own read-site grep provably cannot find a stamper (zero hits on `adapter.ts` — a proof that could not fail). Record: `docs/progress/cutdown-product-program-stage-0b3-plan-review.md` |
| 2026-08-11 | **Stage 0B-3 build gate**: entry gate + `code-reviewer` + `cutdown-boundary-reviewer` + `cutdown-measurement-reviewer` | **NEEDS CHANGES ×3 (round 1, no BLOCK) → PASS + NEEDS CHANGES ×2 (round 2) → closed by class sweep** | Entry gate green first (mid-stage snapshot, before the code gate added 3 fixtures and 2 tests): 984 = 979 + 5 + 0; 689 Python (delta 0); 47 cases / 0 disagreements; status byte-identical. Build deviations all named in the review record — including the F-O falsehood's **fourth home** (`skills-sync.test.ts:116`, found as a red test by the entry gate) and the vacuous-pathspec check catching its own first invocation |
