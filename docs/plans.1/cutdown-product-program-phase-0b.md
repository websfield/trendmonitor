...
**Discharging the master plan's order-critical constraint [R1-fix, tightened R2].** The master plan's §7 Stage-0 **Order-critical** clause says *"the contract bump lands before real-output accumulation resumes, or the accumulation invalidates itself."* Rev 1 deferred the bump without addressing this. **Resolution: accumulation is safe to resume now, with a stated threshold.**
...
> **Correction (2026-08-10, found by `cutdown-measurement-reviewer` at the 0B-3 plan gate).** This file cites "roughly 4×" against `todos.md:26` in four places (the Revision-4 BLOCK-8 row, this section, task 6b and task 10). Those citations are **dead**: T-1 graduated into D-56 and its `todos.md` rows are gone (`:26` now lands inside T-3), and the 4× was an owner estimate of *jobs per output* whose only on-disk measurement says **2×** (`stage-0b-review.md:77` — the same invented figure Stage 0B's review deleted from the PRD and tech-spec). The 4× was never load-bearing here: the ≤3 threshold below stands on the 30%-of-window argument alone, and was re-verified live at 0B-3 landing (window held 1). The four stale citations are retired by this note rather than edited in place, because rewriting a plan's review-history rows would falsify the record of what the gates actually saw.

**The threshold, stated rather than left implicit [R2-fix], and derived rather than asserted [R3-fix]:** the cost of 0B-3's bump grows monotonically with every real output produced in the interim, and under T-1 the window fills more slowly (packages ≠ outputs; the measured ratio on disk is 2× — see the correction above), so it stays cheap for longer — but not forever. **0B-3 must land before criterion 3's window holds more than three resolved real outputs** — the same unit task 10 uses for the threshold.
...
> **Superseded 2026-08-10 — the re-plan happened.** The trigger fired and this outline was discharged by `docs/plans/cutdown-product-program-phase-0b3.md` (gated READY, built same day). Scope as settled by **D-62**: `render-v2` only; `content-package-v2` was never created. This outline stays for provenance; do not implement from it.

**Its first task is to re-read the master plan's order-critical constraint** (the §7 Stage-0 **Order-critical** clause) and decide whether criterion 3's window is empty enough to bump. What the spike already settled:
