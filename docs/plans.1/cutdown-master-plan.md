...
**Owner-blocked:** `PHASE_3_ACCEPTED_LIVE` (D-21: spend ceiling) — still unset.

**~~Neither is reachable from this repo.~~ — CORRECTED 2026-08-09.** That claim was falsified on 2026-08-05, when the owner supplied real campaign footage and directed its use in-session. D-27/D-36 were unblocked for the first real job and `cutdown/docs/proving-run-real.md` records it **DONE**: ContentPackage `01KZ8B40TENCWQ72F061FXK79S`, a 27.8 s vertical showcase cut from six creator assets, `releaseState: rights_approved`, `sourceClassification: real`, QA `pass_with_waivers` (0 blockers), rights manifest covering all six assets, range validation `ran`.

`PHASE_0_EXIT_EARNED` remains **not earned** — but for a different and smaller reason than "unreachable": it now needs *more* real outputs and *more* accounts, not the first of either. Per the owner decision of 2026-08-09 (`todos.md` T-1), an output is one approved cut per `CreativeBrief`, so the two delivered real packages — which share a `creativeBriefId` — count as **1 of 20**, across **1 of 3** accounts.

> **The tool does not yet agree with that number, and this is where it says so.** `cutdown status --phase0` today prints **2/20**: it counts delivered packages, and T-1's supersession rule is not implemented (`status.ts`, criterion `approved-real-outputs`). Implementing it is **Stage 0B task 13**, and the decision that settles it (**D-56**) is reserved and unwritten — so until then, 1 is the *policy* answer and 2 is the *measured* one.
>
> **The consequence is larger than a display count, and is stated rather than left to be discovered.** Criterion 3 ("the last 10 outputs require no breaking contract change") currently reports **green**, and it does so *only* because the tool counts those two correlated packages as two independent outputs — its predicate is satisfied by a window of two, which is itself a known defect Stage 0B task 13 fixes. Under T-1 the window holds **one** output, and criterion 3 becomes honestly **UNPROVEN**. So a criterion that reads green today is expected to go red when the counting model lands. That is the correct direction: the gate gets stricter and the number gets more honest, which is the whole point of this work package.

Continuing work is planned in **`docs/plans/cutdown-product-program-master-plan.md`** (Stages 0–7). That plan does not supersede or renumber this one: this file remains the record of the PRD Phase 0 build.

*Status of the "Open, with reasons" list above, as of 2026-08-09:* the **cutdown ruff selection** is no longer open — Stage 0A widened it (**D-58**). The other three remain open in the receiving stages named in that plan's §6 Deferral Ledger.
