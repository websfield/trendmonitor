...
| **0B** | **Not started — spike first** | Tasks 3–14 (the contract migration). Per §7a: write the failing tests and type signatures and let the compiler and `validate:contracts` answer what three rounds of prose review got wrong, then write the plan from what the code proved |
