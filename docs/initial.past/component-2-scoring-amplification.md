...
**Does not talk to Component 4 at all.** Per [ADR-0007](adr/0007-the-knowledge-api-boundary.md), C2 never calls the Knowledge API and never reads a `Mechanism`. This is the same rule as *C2 never calls C1*, applied to the surface that would otherwise make it easy to violate: a mechanism's evidence is `Proxy`-selected, so retrieving one into a score launders Proxy provenance into a number a client reads, and it makes a VPS irreproducible from its pinned `(extractor × rubric × pattern_library)` triple. The rule is enforceable only because C2 has no path to a mechanism.

**The property that shapes the phasing:** the compliance gate depends on nothing. No library, no breaker, no pattern, no mechanism. It runs with Components 1, 3, and 4 dark, which is why Phase 1 ships it alone.
...
It never calls Component 1 or Component 4. It never reads a `Mechanism`, and no mechanism — or anything derived from one — is ever an input to a veto, a verdict, a VPS, an AWS term, or a budget allocation. It never writes a breaker flag. It never lets model output clear a veto, assign a verdict, or allocate budget. It never scores against an incompatible library. It never auto-approves. It never sums organic and boosted engagement. It never treats an unreachable referee as permission. And it never shows a client a number that Component 3 has not certified.
