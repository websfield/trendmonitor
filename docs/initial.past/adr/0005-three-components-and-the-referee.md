...
**Amended by:** [ADR-0007](0007-the-knowledge-api-boundary.md), which adds a fourth component — a read-only Knowledge API holding no tenant data

---

> **Amendment (ADR-0007, 2026-07-10).** There are four components. C4, the Knowledge API, *serves beliefs*: it reads published `MechanismLibraryVersion` artefacts and returns them over an authenticated HTTP surface. It writes nothing, anywhere.
>
> Every authority statement below is unchanged and unextended. C3 still holds exactly two write authorities and gains no third — a mechanism makes no numeric prediction and touches no outcome data, so there is nothing for a calibration referee to referee. **C4 does not read Contract C at all.**
>
> The call-graph rule gains one line, and it is the same line: **C2 never calls C4.** A scoring path that reads a mechanism has laundered `Proxy`-selected evidence into a client-facing number and has made a VPS irreproducible from its pinned version triple. C4 also never calls C1, C2, or C3; its entire read grant is one artefact-store prefix.
