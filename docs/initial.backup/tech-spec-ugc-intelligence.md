...
Runs on a schedule over the union of the exemplar corpus and the internal labelled corpus, per `(vertical, platform)`.

Candidate patterns are proposed as feature predicates. For each, compute the lift in 24h engagement-rate percentile for posts satisfying the predicate versus the cohort median, with a bootstrapped confidence interval. Patterns clearing the sample-size and CI thresholds in the eval plan are promoted to `active`; the rest sit at `insufficient_evidence`.

Two guards. First, multiple-comparison correction: mining a hundred candidate predicates against a few hundred posts will surface several spurious "patterns" at p < 0.05 by construction. Benjamini-Hochberg across the candidate set, and a hard requirement that a pattern replicate in a held-out temporal split before promotion. Second, every promoted pattern is back-tested against the prior quarter's data. A pattern that only holds in the window it was mined from is a description of that window, not a pattern.

Explore-arm outcomes enter this mining step with equal weight to exploit-arm outcomes. This is the entire economic justification for the exploration budget: it is the only source of unbiased evidence about content the exploit policy would never have selected.
...
GET    /api/patterns?vertical=&platform=&status=active
```
...
POST   /internal/mine       {vertical, platform, window} → candidate patterns with CIs
