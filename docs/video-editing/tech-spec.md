...
**Self-rooted workspace.** `cutdown/` carries its own `pnpm-workspace.yaml`, `package.json`, and `pyproject.toml` (uv workspace). Nothing above `cutdown/` references it, and it references nothing above itself; all commands run as `pnpm -C cutdown ...` (or from inside the directory). This is what makes the Stage C extraction a directory copy. The parent repo's build systems (npm frontend, dotnet, root uv) are untouched.
...
- **Entry gate:** `cutdown build:contracts --check` + `cutdown validate:contracts` + `cutdown test:skills` run before anything else. Generated trees must be current, schemas parse, and fixtures pass through both validators.
...
- **Render determinism is a three-tier assertion**, replacing any single byte-identical claim (this is what PRD REQ-080's "documented determinism limits" hedge concretely means — and this section is where those limits are documented):
  1. **Byte-identical** — `renderer-ffmpeg` only, on the pinned local environment (D-33 — no CI exists at Phase 0): pinned FFmpeg build, fixed `threads=N`, `-fflags +bitexact -flags +bitexact -map_metadata -1`, `creation_time` stripped, pinned audio encoder. x264 is bit-exact under these constraints; the test asserts it.
...
- The Critical-Path reviewer gates (`veto-verdict-integrity`, `component-boundaries`, `measurement-discipline`, `budget-exploration`) do not apply to Cutdown changes, and **cutdown-only changes are exempt from the UGC Intelligence entry-gate commands** (`dotnet build/test`, root `pytest`, frontend checks) — Cutdown's gates are §12's commands plus ordinary code review. Working agreements (branching, review-by-whom, commit cadence) live in `developer-guide.md`.
- If and when Cutdown is extracted to its own repository (Stage C), the only shared artefact that needs to survive the split is this doc set — no code coupling exists to unwind.
