...
**Phase 0 — the population the numbers are over (D-56).** The numerals are unchanged, but they are not counted in delivered packages. **"At least 20 approved real outputs" and "last 10 outputs" are both counted in *resolved outputs*: one approved cut per `CreativeBrief`, where a second delivered package for the same CreativeBrief supersedes the earlier one rather than adding.** The two are not interchangeable, and reading the 20 as packages would understate the required work — but **no multiplier is asserted here**, because none has been measured. The only ratio on disk today is that the two delivered `real` packages resolve to **one** output. (When the owner settled T-1 they estimated the definition swings the gate between *~5 jobs and ~20 jobs* of real production work; that is an estimate of jobs per output, it is recorded in D-56, and it is not a packages-per-output ratio.) The other two Phase 0 criteria are denominated differently and deliberately: "zero invalid source ranges in final renders" and "rights records and QA reports accompany every delivered package" are counted in **delivered packages**, and neither carries a "real" qualifier, so both legitimately include fixture packages. §14.2's "30 comparable **published** outputs" is a third population again — published outputs, which Phase 0 does not yet produce at all.

The counting rule itself, the class tabulation (variant / CreativeBrief revision / downstream revision / repackage / rerender / superseded), the real-versus-fixture split per criterion, and the comparability axes every later uplift claim rests on have exactly one home: **`docs/video-editing/output-counting-policy.md`**. This roadmap names the population kind and cites it; it does not restate the rule.
