...
**Machine-readable:** [schemas/rubric-v1.json](schemas/rubric-v1.json) *(rubric_version `1.1.0`)*
...
> **What may never be a term in any score here.** Neither a `TrendSignal` ([ADR-0004](adr/0004-trend-detection-and-submission.md)) nor a `Mechanism` ([ADR-0006](adr/0006-mechanisms-and-the-warrant-ladder.md)) enters VPS, BAS, or AWS at any weight, under any configuration. Both refresh on cadences a temporally held-out split cannot survive, and a mechanism's evidence is `Proxy`-selected — retrieving one into a score launders that provenance into a number a client reads. Component 2 has no code path to either, which is what makes the rule enforceable rather than aspirational. `schemas/rubric-v1.json` records this machine-readably under `scoring_inputs_forbidden`.
>
> **Naming note (rubric 1.1.0).** `creator_standing`'s source is the **C³ framework's ACE score** (Creator / Content / Campaign), and the schema field is now `ace_creator_score`. It was `c3_ace`, written before [ADR-0005](adr/0005-three-components-and-the-referee.md) named the Calibration Monitor **C3** — after which it read as *"this AWS term is sourced from Component 3,"* which would be a boundary violation. C2's only read path to C3 is Contract C, the breaker flag. No weight, gate, or threshold changed.
