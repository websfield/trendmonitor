...
> **Amended.** This section was written as two components. There are four. [ADR-0005](adr/0005-three-components-and-the-referee.md) adds **C3, the Calibration Monitor** — the referee, holding sole breaker authority and sole veto over pattern-library promotion, because neither of the components below can be allowed to grade its own homework. [ADR-0007](adr/0007-the-knowledge-api-boundary.md) adds **C4, the Knowledge API** — read-only, holding no tenant data, serving the `Mechanism` artefact that answers *why* rather than *how much*. Read [integration-contract.md](integration-contract.md) for how the four talk. The two components described below are unchanged.

Two components do the visible work, deliberately separated, connected by a single versioned artefact.
...
**REQ-006** [Must] When a pattern's supporting evidence ages past its validity window without refresh, the system flags it as stale and excludes it from scoring rather than silently continuing to apply it. The equivalent rule for trend signals is **REQ-005h** in the trend subsystem spec. *(That rule was previously also numbered REQ-006, colliding with this one. It has been renumbered; this requirement is unchanged.)*

**REQ-007** [Could] The engine surfaces a quarterly "what changed" report per client vertical: patterns that strengthened, patterns that decayed, and new patterns that cleared the evidence threshold. Superseded in framing by **REQ-070**: the report is *derived by reading the Knowledge API* rather than assembled independently, so that a mechanism falsified in week two does not sit in a printed report for eleven more.

**REQ-008** [Must] Pattern mining runs in two stages reading two different corpora. **Proposal** of candidate feature predicates runs over the union of the exemplar corpus and the internal labelled corpus. **Estimation** of effect sizes runs over the internal labelled corpus only. No `Proxy`-provenance value enters an effect-size calculation, at any weight, under any configuration. See [ADR-0001](adr/0001-trend-signal-sourcing.md) and [ADR-0006](adr/0006-mechanisms-and-the-warrant-ladder.md).

### Knowledge Layer

Full specification in [tech-spec-knowledge-layer.md](tech-spec-knowledge-layer.md), decided in [ADR-0006](adr/0006-mechanisms-and-the-warrant-ladder.md) and [ADR-0007](adr/0007-the-knowledge-api-boundary.md), where REQ-060 through REQ-070 are defined. In summary:

The system maintains a `Mechanism` — a falsifiable hypothesis about **why** a content structure recurs among high performers — mined exclusively from the public exemplar corpus and from trend signals, keyed by `(vertical, platform)` with **no tenant axis**. A mechanism carries a required `falsifier`, a `warrant` rung computed deterministically from corpus counts, and **no effect size, by schema**. Warrant demotion is automatic and immediate; promotion requires a named human to ratify the model-drafted statement.

Mechanisms are served by **Component 4**, a read-only, tenant-authenticated API holding no tenant-scoped data — which is the only reason it can be exposed outside ClientHub. No mechanism, and nothing derived from one, is ever an input to a veto, a verdict, a VPS, an AWS term, or a budget allocation.
...
**Mechanism survival rate.** Proportion of `contrasted` mechanisms that survive the next corpus refresh without demoting to `falsified`. There is no target, and setting one would be a mistake — a survival rate of 1.00 means the refresh is not testing anything, and a rate near zero means the `contrasted` bar is mining noise. This number is *reported*, watched, and used to argue about the thresholds; it is never optimised. The one thing it must not be is invisible: a quarterly report saying "three mechanisms were falsified" reads to a client as the system having been wrong, and it is the system working.
...
This is also **not** what the Knowledge API does, and the distinction is the whole of [ADR-0006](adr/0006-mechanisms-and-the-warrant-ladder.md). A `Mechanism` crosses tenants because it never contained a tenant: it is mined exclusively from the public exemplar corpus, and no `OutcomeEvent`, `Pattern`, or operational table is reachable from the synthesiser. What is also out of scope, and was seriously considered: **pooling per-tenant effect sizes by meta-analysis**, or publishing a count of "3 of 5 tenants confirmed this." A summary statistic of outcome data is outcome data at lower resolution, and at five tenants with distinguishable verticals it is re-identifiable in practice.

**A causal claim about content.** The Knowledge API's top rung is `contrasted` — *this structure recurs among high performers and is materially absent from the same creators' non-performers*. It is not *this structure causes performance*. Reaching a causal rung would require either internal outcome data in a tenant-neutral artefact (forbidden above) or deliberately stratifying the exploration budget on a content predicate (which trades away the objective [ADR-0003](adr/0003-exploration-budget.md) chose, for power this data volume does not have). Both rungs are named in the warrant ladder and both are refused, because a ladder whose top is invisible gets climbed by accident.
...
**Phase 6 - Mechanism synthesis and the Knowledge API (4 weeks, and it can start at Phase 2).** Mechanisms are synthesised from the exemplar corpus and its trend-directed refreshes. Component 4 ships read-only behind tenant authentication. **The API returns empty collections, per cohort, for months** — until eight independent creators across two cohorts and two unrelated trends carry a predicate, nothing clears `recurrent`, and nothing is served. That is the correct early state and the `coverage` object is what stops it reading as a broken one.

This phase depends on the exemplar corpus (Phase 2) and on the trend subsystem, and on **nothing else**. It does not need the scorer, the breaker, or a single outcome event, because a mechanism is mined from public content and touches no tenant data. It could ship before Phase 3. The reason to sequence it here is attention, not dependency.

The temptation to accelerate it by feeding internal outcomes into a mechanism is the temptation this design exists to refuse.
