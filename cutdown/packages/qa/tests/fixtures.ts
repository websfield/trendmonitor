/**
 * Schema-valid fixture builders for the QA gate tests.
 *
 * `runDeterministicGates` runs the EDL through `resolveEdl`, which validates it
 * against `platform-edl-v1`, so these builders must produce fully schema-valid
 * artefacts — a fixture that drifts from the schema would turn a green gate test
 * into a lie. Modelled on `packages/editorial/tests/fixtures.ts`.
 */
...
function envelope(skill: string): { schemaVersion: string; createdAt: string; createdBy: { kind: 'skill'; skill: string; skillVersion: string } } {
  return { schemaVersion: '1.0.0', createdAt: '2026-07-28T00:00:00Z', createdBy: { kind: 'skill', skill, skillVersion: '1.0.0' } };
}

/** The TikTok organic AU capability (PRD §11 worked example + D-3 pins). */
export const TIKTOK_CAPABILITY: PlatformCapability = {
  platform: 'tiktok',
...
export function boundsMap(): ReadonlyMap<string, AssetBounds> {
  return new Map([[ASSET_ID, { assetId: ASSET_ID, duration: { ticks: 900, timebase: { num: 1, den: 30 } } }]]);
}

export function makeJobBrief(overrides: Partial<JobBriefV1.JobBrief> = {}): JobBriefV1.JobBrief {
  return {
    briefId: ulid(0),
    envelope: envelope('brief'),
    accountId: 'acct-1',
...
export function makeMoment(overrides: Partial<MomentV1.Moment> = {}): MomentV1.Moment {
  return {
    ...
    transcript: { verbatimText: 'we cut the price in half this month', displayText: 'We cut the price in half.', wordCount: 8, segmentIds: ['seg1'], lowConfidenceWordCount: 0 },
...
export function makeStyleProfile(overrides: Partial<StyleProfileV1.StyleProfile> = {}): StyleProfileV1.StyleProfile {
  return {
    styleProfileId: ulid(6),
    envelope: envelope('style'),
    accountId: 'acct-1',
    profileVersion: '1.0.0',
    approval: null,
    colours: [{ name: 'Ink', hex: '#141414', role: 'text' }],
    fonts: [{ family: 'Inter', role: 'caption', rightsRecorded: true }],
    logoRules: { mustAppear: false, allowedPlacements: [], prohibitedTreatments: [] },
    toneOfVoice: { descriptors: ['plain-spoken'], casing: 'sentence', emojiUse: 'none', allowProfanity: false },
    prohibitedClaims: ['algorithm hack'],
    prohibitedTreatments: ['no fear-based hooks'],
    ...overrides,
  };
}
