...
- **Account & external-dependency continuity:** are the outside accounts the company depends on (registrar/DNS, hosting, transactional email, payment processor) recorded in `RUNBOOK.md`'s **Accounts** section (or an equivalent a stranger could find) — where the login lives, when things renew, and when the inventory was last reviewed? (A dead domain or a payment account tied to a lost email kills a solo company as surely as a code bug. Where an account inventory exists, audit its staleness — a "last reviewed" date going stale is itself the finding, since nothing here monitors renewals for real.)
